...
#if defined(FREENOVE_FNK0104B)
static void initCsi() {
    // Set up ESP32 as its own open AP so the WiFi NIC has frames to measure.
    // No password, no clients needed — the NIC captures its own beacons + nearby
    // WiFi traffic, giving CSI data for motion detection without any network auth.
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_AP));

    wifi_config_t ap_cfg = {};
    strcpy((char*)ap_cfg.ap.ssid, "RADAR");
    ap_cfg.ap.ssid_len = 4;
    ap_cfg.ap.channel = 6;              // fixed channel for CSI stability
    ...
    wifiReady = true;
    enableCsi();
    Serial.println("# CSI active (AP mode, channel 6)");
}
#else
// Cardputer: CSI only, no WiFi connection needed
static void initCsi() {
    wifiReady = true;
    enableCsi();
...
static void serviceKeys() {
    ...
    if (gMenuOpen) {
        ...
        if      (zone == 1) { gThreshold -= 0.05f; if (gThreshold < 0.05f) gThreshold = 0.05f; radar.setThreshold(gThreshold); }
        else if (zone == 2) { gThreshold += 0.05f; if (gThreshold > 0.95f) gThreshold = 0.95f; radar.setThreshold(gThreshold); }
        else if (zone == 3) startCalibration();
...
static void serviceKeys() {
    ...
    for (char c : st.word) {
        if (gMenuOpen) {
            ...
            else if (c == ',') { gThreshold -= 0.05f; if (gThreshold < 0.05f) gThreshold = 0.05f; radar.setThreshold(gThreshold); }
            else if (c == '/') { gThreshold += 0.05f; if (gThreshold > 0.95f) gThreshold = 0.95f; radar.setThreshold(gThreshold); }
        }
    }
}
#endif
